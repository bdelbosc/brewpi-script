...
# Contributors:
#   Benoit Delbosc
""" lib to send BrewPi metrics to InfluxDB
"""
...
class BrewMon(object):
    ...
    def publish_line(self, beer_name, line):
        if line and beer_name:
            row = line.split(';')
            self.publish_row(beer_name, row)

    def publish_row(self, beer_name, row):
        if len(row) not in [7, 9]:
            print("Invalid row expecting 7 or 9 columns: " + str(row))
            return
        try:
            point = get_beer_point(beer_name, row)
        except ValueError as e:
            print("Invalid line: " + str(row) + " " + str(e))
            return
        if self.verbose:
            print(point)
...
def get_beer_point(beer_name, row):
    ...
    if check_row(row, 2):
        fields['beer_setting_temp'] = float(row[2])
    if check_row(row, 4):
        fields['fridge_temp'] = float(row[4])
    if check_row(row, 5):
        fields['fridge_setting_temp'] = float(row[5])
    if check_row(row, 8):
        fields['room_temp'] = float(row[8])
    if check_row(row, 7):
        fields.update(get_states(row[7]))
    fields.update(get_beer_annotation(row[3], row[6]))
    return {
...
def get_beer_annotation(text, text2):
    if text and text != 'null':
        return {"title": text, "tags": "brewpi"}
    if text2 and text2 != 'null':
        return {"title": text2, "tags": "brewpi"}
    return {}
...
def get_utc_datetime(time):
    ...
    tzname = LOCAL_TIME.tzname(dt)
    # CEST does not exist
    local_tz = timezone(tzname.replace('CEST', 'Europe/Paris'))
